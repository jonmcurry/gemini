...
class ClaimValidator:
    ...
    def validate_claim(self, claim: ProcessableClaim) -> List[str]:
        ...
        # Rule: claim_id must be present (already handled by Pydantic if not Optional)
        if not claim.claim_id: # Should not happen if Pydantic model is used correctly
            errors.append("Missing claim_id.")

        # Rule: facility_id must be present
        if not claim.facility_id:
            errors.append("Missing facility_id.")

        # Rule: patient_account_number must be present
        if not claim.patient_account_number:
            errors.append("Missing patient_account_number.")
        ...
        # Rule: Sum of line item charges should ideally match total_charges (optional, can be complex)
